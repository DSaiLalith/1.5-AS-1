...
app.get('/cart-total', (req, res) => {
  ...
  let newItemPrice = item1Price + item2Price + item3Price;
...
function memberShipDiscount(isMember, cartTotal) {
  if (isMember === 'true') {
    return cartTotal - (cartTotal / 100) * 5;
  } else {
...
app.get('/membership-discount', (req, res) => {
  ...
  let cartTotal = parseFloat(req.query.cartTotal);
  res.send(memberShipDiscount(isMember, cartTotal));
});
...
app.get('/calculate-tax', (req, res) => {
  let cartTotal = parseFloat(req.query.cartTotal);
  let result = cartTotal + (cartTotal / 100) * 5;
  res.send(result.toString());
});

// Endpoint 4 : Estimate delivery time based on shipping method

function estimateDelivey(shippingMethod, distance) {
...
app.get('/estimate-delivery', (req, res) => {
  ...
  res.send(estimateDelivey(shippingMethod, distance));
});
...
function calculateloyaltyPoints(purchaseAmount) {
  let loyaltyPoints = purchaseAmount * 0.2;
